...
class MyDataLoader:
    def __init__(self, cfg):
        preprocessor = Preprocessor(cfg)#把“读取和处理数据”的任务外包给了 Preprocessor（预处理器）。
        self.tokenizer = AutoTokenizer.from_pretrained(cfg.bert_path)
        
        self.data = preprocessor.forward()# 2. 命令预处理器：“去把数据给我拿回来！”
    ...
    def collate_fn(self, lst):
        if 'dep' in self.config.input_files:
            ...
            token_index, thread_length, token2speaker, reply_mask, speaker_mask, thread_mask, pieces2words, new2old, \
                ...
                adj_matrixes, dag_adj_list, dag_s_mask_list, utterance_level_speaker_adj = zip(*lst)
        else:
            doc_id, speaker_ids, input_ids, input_masks, input_segments,sentence_length, token2sents, utterance_index, \
            token_index, thread_length, token2speaker, reply_mask, speaker_mask, thread_mask, pieces2words, new2old, \
                triplets, pairs, entity_list, rel_list, polarity_list, thread_idxes,\
        ...
        if 'dep' in self.config.input_files:
            ...
            max_sent_num = max(sentence_nums)
            dag_adj_np = np.zeros((len(sentence_nums), max_sent_num, max_sent_num), dtype=np.float32)
            dag_s_mask_np = np.zeros((len(sentence_nums), max_sent_num, max_sent_num), dtype=np.int64)
            for i in range(len(sentence_nums)):
                n_i = dag_adj_list[i].shape[0]
                dag_adj_np[i, :n_i, :n_i] = dag_adj_list[i]
                dag_s_mask_np[i, :n_i, :n_i] = dag_s_mask_list[i]

            # 6. 处理句子级其他辅助信息
            utterance_level_speaker_adj_np = np.zeros((len(sentence_nums), max_sent_num, max_sent_num), dtype=np.float32)
            for i in range(len(sentence_nums)):
                n_i = utterance_level_speaker_adj[i].shape[0]
                utterance_level_speaker_adj_np[i, :n_i, :n_i] = utterance_level_speaker_adj[i]
            
            utterance_level_mask = np.zeros([len(sentence_nums), max_sent_num], dtype=np.float32)
            for i in range(len(sentence_nums)):
                utterance_level_mask[i, :sentence_nums[i]] = 1
        ...
        if 'dep' in self.config.input_files:
            items = {
                "adj_matrixes": new_adj_matrixes, 
                'global_masks': global_masks,
                'dag_adj': dag_adj_np, # 新增项
                'dag_s_mask': dag_s_mask_np, # 新增项
                'utterance_level_speaker_adj': utterance_level_speaker_adj_np, 
                'utterance_level_mask': utterance_level_mask,
            }
            res.update(items)
